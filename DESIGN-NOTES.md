...
A remaining issue with all of this, is that if the on-screen cursor is not at the end of the input line, ProDOS will erase whatever's in the line after the cursor position when it steals the command, via a call to `ClrEol`. The actual contents of the input line remain intact, but the on-screen display of the line gets truncated. This is because, while we set the X register to the end of the input buffer, we still have to leave the on-screen cursor position (as described by `CH` and `CV`: `$24` and `$25` on the zero page) wherever it really is within the input line, or else the flashing cursor will be at the wrong spot and the user won't be able to tell where typed characters would appear.

This issue could be worked around, by instructing our ProDOS-wrapped `KSW` routine to never directly return a carriage return, but instead communicate it via a side-channel to our prompt while returning a harmless character (that the prompt knows to ignore). If ProDOS's wrapper doesn't *see* a carriage return, it can't *react* to one. It would give our prompt the opportunity to do cleanup, and set the on-screen cursor to the end of the line. It could then call `KSW` again, this time instructing our input hook to immediately return a carriage return without actually fetching a user keypress, allowing ProDOS its opportunity to steal-and-execute the command now that we've prepared for it to do so. But this idea ony occurred to me while I was writing this document up, so I haven't yet implemented it.
